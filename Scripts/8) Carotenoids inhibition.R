...
data %>% 
  ggplot() +
  geom_point(mapping = aes(x = Concentration, y = Carotenoids_inhibition), 
             size = 2, alpha = 0.5) +
  labs(x = "Diuron (mg/L)", 
       y = "Carotenoids inhibition (%)") +
  theme_light()
...
data %>% 
  ...
  lm(formula = Carotenoids_inhibition ~ Concentration, 
...
carotenoids_inhibition.drm <- data %>% 
  drm(formula = Carotenoids_inhibition ~ Concentration, data = .,
      fct = LL.4(names = c("Slope", "Lower Limit", "Upper Limit", "EC50")))

## Dose-response parameters

carotenoids_inhibition.drm %>% 
...
data.frame(Concentration = seq(from = min(data$Concentration),
                               ...
                               length.out = 1000)) %>% 
  mutate(fit = predict(carotenoids_inhibition.drm, newdata = .), 
         lwr = predict(carotenoids_inhibition.drm, newdata = ., interval = "confidence", vcov. = sandwich)[, 2], 
         upr = predict(carotenoids_inhibition.drm, newdata = ., interval = "confidence", vcov. = sandwich)[, 3]) %>% 
  ggplot() +
  geom_ribbon(mapping = aes(x = Concentration, ymin = lwr, ymax = upr), alpha = 0.2) +
  geom_line(mapping = aes(x = Concentration, y = fit), size = 1) +
  geom_point(mapping = aes(x = Concentration, y = Carotenoids_inhibition), data = data, size = 2, alpha = 0.5) +
  labs(x = "Diuron (mg/L)", 
       y = "Carotenoids inhibition (%)") +
  theme_light()
...
ggsave(filename = "Figures/8) Carotenoids inhibition.png", height = 5.25, width = 7, 
